...
export const getDeadline = createServerFn({ method: 'GET' }).handler(async (): Promise<{ deadline: string | null }> => {
...
export const setDeadline = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<{ ok: boolean; deadline: string | null; error?: string }> => {
    ...
    if (!session.role || !['superadmin', 'admin'].includes(session.role)) {
      return { ok: false, deadline: null, error: 'Hanya admin yang bisa mengatur tenggat penilaian' }
    }
    const database = assertDb()
    const [comp] = await database.select({ id: competitions.id }).from(competitions).where(eq(competitions.slug, DEKOR_5R_SLUG)).limit(1)
    if (!comp) return { ok: false, deadline: null, error: 'Lomba dekor-5r tidak ditemukan' }

    const deadline = data.deadline ? new Date(data.deadline) : null
    if (deadline && Number.isNaN(deadline.getTime())) return { ok: false, deadline: null, error: 'Tanggal tenggat tidak valid' }
  ...
  .handler(async ({ data }): Promise<{ ok: boolean; id?: string; error?: string }> => {
    ...
    if (data.formId === DEKORASI_FORM_ID && createdBy) {
      ...
      const dup = await database
        ...
        .where(and(eq(fiveRSubmissions.roomId, data.roomId), eq(fiveRSubmissions.formId, DEKORASI_FORM_ID), eq(fiveRSubmissions.createdBy, createdBy)))
        .limit(1)
      if (dup.length) return { ok: false, error: 'Ruangan ini sudah dinilai (lomba dekorasi). Satu ruangan hanya bisa dinilai sekali per auditor.' }
    }
    ...
    try {
      ...
      if (isUniqueViolation(err)) {
        return { ok: false, error: 'Ruangan ini sudah dinilai (lomba dekorasi). Satu ruangan hanya bisa dinilai sekali per auditor.' }
      }
      throw err
    }
...
export const deleteSubmission = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<{ ok: boolean; error?: string }> => {
    ...
    const db = assertDb()
    const [row] = await db.select().from(fiveRSubmissions).where(eq(fiveRSubmissions.id, data.id)).limit(1)
