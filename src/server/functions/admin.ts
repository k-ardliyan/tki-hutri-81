...
export const createUser = createServerFn({ method: 'POST' })
  .validator((d: { username: string; password: string; role: UserRole; employeeId?: number | null }) => d)
  .handler(async ({ data }) => {
    const db = assertDb()
    const passwordHash = await hashPassword(data.password)
    const [row] = await db
      .insert(users)
      .values({ username: data.username, passwordHash, role: data.role, employeeId: data.employeeId ?? null })
      .returning()
    return row
  })
...
export const resetPassword = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    const db = assertDb()
    const passwordHash = await hashPassword(data.password)
    const [row] = await db.update(users).set({ passwordHash }).where(eq(users.id, data.id)).returning()
    return row
  })
...
export const createEmployee = createServerFn({ method: 'POST' })
  .validator((d: { nama: string; nip?: string | null; divisi?: string | null; isSnackEligible?: boolean }) => d)
  .handler(async ({ data }) => {
    const db = assertDb()
    const [row] = await db
      .insert(employees)
...
export const updateEmployee = createServerFn({ method: 'POST' })
  .validator((d: { id: number; nama?: string; nip?: string | null; divisi?: string | null; isSnackEligible?: boolean }) => d)
  .handler(async ({ data }) => {
    const db = assertDb()
    ...
    const [row] = await db.update(employees).set(patch).where(eq(employees.id, data.id)).returning()
    return row
  })
