  ...
  .handler(async ({ data }) => {
    ...
    if (nomor !== null) {
      const dup = await database
        ...
        .limit(1);
      if (dup.length)
        throw new Error(`Nomor ${nomor} sudah dipakai kategori ${labelKategori(data.kategori)}`);
    }
    const kode =
      data.kode?.trim() ||
      (data.kategori === 'panitia' ? 'PANITIA' : `${data.kategori.toUpperCase()}-${nomor}`);
    try {
      const [row] = await database
        .insert(teams)
        .values({ kategori: data.kategori, nomor, nama, kode })
        .returning();
      return row;
    } catch (err) {
...
export const updateTeam = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    if (kategori !== 'panitia' && nomor === null)
      throw new Error('Nomor tim wajib diisi untuk kategori ini');
    ...
    if (kategori !== 'panitia' && nomor !== null) {
      const dup = await database
        ...
        .limit(1);
      if (dup.length)
        throw new Error(`Nomor ${nomor} sudah dipakai kategori ${labelKategori(kategori)}`);
    }
