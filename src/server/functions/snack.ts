...
import { teams, teamMembers, employees, snackSessions, redemptions } from '../db/schema'
...
export const createSession = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    const db = assertDb()
    const [row] = await db.insert(snackSessions).values({ name: data.name, quota: data.quota, isActive: true }).returning()
    return row
  })
...
export const updateSession = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const [row] = await db.update(snackSessions).set(patch).where(eq(snackSessions.id, data.id)).returning()
    return row
  })
...
export const getTeamsWithMembers = createServerFn({ method: 'GET' }).handler(async (): Promise<SnackTeam[]> => {
  const db = assertDb()
...
export const getTeamByKode = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }): Promise<SnackTeam | null> => {
    ...
    return {
      ...
      members: members.map((m) => ({ employeeId: m.employeeId, nama: m.nama, nip: m.nip, divisi: m.divisi })),
    }
  })
...
export const searchEmployees = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }): Promise<Array<{ id: number; nama: string; nip: string | null; divisi: string | null }>> => {
    const q = data.q.trim()
    if (!q) return []
    const db = assertDb()
    const like = `%${q}%`
    const rows = await db
      .select({ id: employees.id, nama: employees.nama, nip: employees.nip, divisi: employees.divisi })
      .from(employees)
      .where(and(eq(employees.isSnackEligible, true), sql`(${employees.nama} ILIKE ${like} OR ${employees.nip} ILIKE ${like})`))
      .limit(data.limit ?? 10)
    return rows
  })
...
export const redeemSnack = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }): Promise<RedeemResult> => {
    ...
    const [session] = await db.select().from(snackSessions).where(eq(snackSessions.id, sessionId)).limit(1)
    ...
    const empRows = await db
      .select({ id: employees.id, nama: employees.nama, isSnackEligible: employees.isSnackEligible })
      .from(employees)
    ...
    if (ineligible.length) {
      return { ok: false, inserted: 0, skipped: [], error: `${ineligible.map((e) => e.nama).join(', ')} tidak eligible snack` }
    }

    const existing = await db
      .select()
      .from(redemptions)
      .where(and(inArray(redemptions.employeeId, employeeIds), eq(redemptions.sessionId, sessionId)))

    const existingIds = new Set(existing.map((r) => r.employeeId))
    const toInsert = employeeIds.filter((id) => !existingIds.has(id))

    if (toInsert.length > 0) {
      await db.insert(redemptions).values(toInsert.map((employeeId) => ({ employeeId, sessionId, claimedBy })))
    }
...
export const getRedemptionSummary = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    if (!active) return { active: null, teams: [], totalRedeemed: 0, totalQuota: 0, sessions: sessionsWithMeta }

    const teamsRows = await db.select().from(teams).orderBy(teams.kategori, teams.nomor)
    const reds = await db
      .select({ employeeId: redemptions.employeeId, claimedAt: redemptions.claimedAt, claimedBy: redemptions.claimedBy })
      .from(redemptions)
...
export const getRedemptionsBySession = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const reds = empIds.length > 0
      ? await db
          .select({ employeeId: redemptions.employeeId, claimedBy: redemptions.claimedBy, claimedAt: redemptions.claimedAt })
          .from(redemptions)
          .where(and(eq(redemptions.sessionId, data.sessionId), inArray(redemptions.employeeId, empIds)))
      : []
