...
import { tournamentService, type TournamentDb } from '../services/tournament'
...
export const getBaganCompetitions = createServerFn({ method: 'GET' }).handler(async () => {
  if (!db) return []
  const rows = await db
    .select({ id: competitions.id, slug: competitions.slug, short: competitions.short, title: competitions.title })
    .from(competitions)
...
export const getBaganTeams = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    if (!db) return []
    const rows = await db.select({ id: teams.id, nama: teams.nama }).from(teams).where(eq(teams.kategori, data.kategori)).orderBy(teams.nomor)
    return rows
  })
...
export const generateBracket = createServerFn({ method: 'POST' })
  .validator((d: { competitionId: number; kategori: Kategori; seedingMethod: SeedingMethod; thirdPlaceEnabled: boolean; manualPositions?: Record<string, number> }) => d)
  .handler(async ({ data }) => {
    ...
    const teamRows = await database
      ...
      .orderBy(teams.nomor)
    const manual = data.manualPositions ? new Map(Object.entries(data.manualPositions).map(([k, v]) => [Number(k), v])) : undefined
...
export const submitMatchResult = createServerFn({ method: 'POST' })
  .validator((d: { matchId: number; winnerId: number; score1?: number | null; score2?: number | null; resultType?: 'NORMAL' | 'WALKOVER' | 'DISQUALIFIED'; notes?: string | null; expectedVersion: number }) => d)
  .handler(async ({ data }) => {
    if (!Number.isInteger(data.winnerId) || data.winnerId < 1) throw new Error('Pemenang tidak valid')
...
export const correctMatchResult = createServerFn({ method: 'POST' })
  .validator((d: { matchId: number; winnerId: number; reason?: string | null; invalidateDownstream: boolean }) => d)
  .handler(async ({ data }) => {
...
export const upsertPrize = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    const database = assertDb()
    if (!Number.isInteger(data.place) || data.place < 1) throw new Error('Juara ke- harus angka positif')
    const where = and(eq(lombaPrizes.competitionId, data.competitionId), eq(lombaPrizes.kategori, data.kategori), eq(lombaPrizes.place, data.place))
    const [existing] = await database.select({ id: lombaPrizes.id }).from(lombaPrizes).where(where).limit(1)
    if (existing) {
      await database.update(lombaPrizes).set({ hadiah: data.hadiah }).where(eq(lombaPrizes.id, existing.id))
    } else {
      try {
        await database.insert(lombaPrizes).values({ competitionId: data.competitionId, kategori: data.kategori, place: data.place, hadiah: data.hadiah })
      } catch (err) {
...
export const deletePrize = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    await database
      .delete(lombaPrizes)
      .where(and(eq(lombaPrizes.competitionId, data.competitionId), eq(lombaPrizes.kategori, data.kategori), eq(lombaPrizes.place, data.place)))
    return { ok: true }
  })
...
export const getPrizes = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    if (!db) return []
    const rows = await db.select().from(lombaPrizes).where(and(eq(lombaPrizes.competitionId, data.competitionId), eq(lombaPrizes.kategori, data.kategori))).orderBy(lombaPrizes.place)
