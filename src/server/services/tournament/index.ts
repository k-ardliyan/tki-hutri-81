...
import { generateBracketStructure, type DerivedSlot, type InitialSlot } from '../../../lib/tournament/generator'
...
import type { BracketDetailView, BracketStatus, MatchStatus, SeedingMethod, TournamentPodium } from '../../../lib/tournament/types'
...
async function loadBracket(database: Database, competitionId: number, kategori: 'putra' | 'putri') {
  const [b] = await database.select().from(brackets).where(and(eq(brackets.competitionId, competitionId), eq(brackets.kategori, kategori))).limit(1)
  return b ?? null
}
...
async function fillSlot(
  ...
  teamId: number | null,
  match: { id: number; participant1Id: number | null; participant2Id: number | null; status: MatchStatus },
) {
  await database.update(bracketMatchSlots).set({ participantId: teamId }).where(eq(bracketMatchSlots.id, slot.id))
...
async function propagateWinner(database: Database, match: { id: number; winnerId: number | null; nextMatchId: number | null; nextMatchSlot: 1 | 2 | null }) {
  if (match.nextMatchId === null || match.nextMatchSlot === null || match.winnerId === null) return
  const [target] = await database.select().from(bracketMatches).where(eq(bracketMatches.id, match.nextMatchId)).limit(1)
  if (!target) return
  const [slot] = await database.select().from(bracketMatchSlots).where(and(eq(bracketMatchSlots.matchId, target.id), eq(bracketMatchSlots.slotNumber, match.nextMatchSlot))).limit(1)
  if (!slot) return
  await fillSlot(database, slot, match.winnerId, target)
}

/** Propagate loser ke slot LOSER_OF (third place). */
async function propagateLoserToThirdPlace(database: Database, match: { id: number; loserId: number | null }) {
  if (match.loserId === null) return
  const [slot] = await database
    .select({ id: bracketMatchSlots.id, matchId: bracketMatchSlots.matchId, slotNumber: bracketMatchSlots.slotNumber })
    .from(bracketMatchSlots)
    .where(and(eq(bracketMatchSlots.sourceType, 'LOSER_OF'), eq(bracketMatchSlots.sourceMatchId, match.id)))
    .limit(1)
  if (!slot) return
  const [target] = await database.select().from(bracketMatches).where(eq(bracketMatches.id, slot.matchId)).limit(1)
  if (!target) return
  await fillSlot(database, slot, match.loserId, target)
}

/** Hitung status bracket baru setelah result. */
async function computeBracketStatus(database: Database, bracketId: number, current: BracketStatus, thirdPlaceEnabled: boolean): Promise<BracketStatus> {
  const matches = await loadMatches(database, bracketId)
  const rounds = await database.select().from(bracketRounds).where(eq(bracketRounds.bracketId, bracketId))
  const mainRounds = rounds.filter((r) => r.roundType === 'MAIN')
  const finalRound = mainRounds.sort((a, b) => a.sortOrder - b.sortOrder)[mainRounds.length - 1] ?? null
  const thirdRound = rounds.find((r) => r.roundType === 'THIRD_PLACE') ?? null
  const finalMatch = finalRound ? matches.find((m) => m.roundId === finalRound.id && m.matchNumber === 1) ?? null : null
  const thirdPlaceMatch = thirdRound ? matches.find((m) => m.roundId === thirdRound.id) ?? null : null
  const hasAnyResult = matches.some((m) => m.status === 'COMPLETED' || m.status === 'AUTO_ADVANCED')
  const finalCompleted = finalMatch ? finalMatch.status === 'COMPLETED' || finalMatch.status === 'AUTO_ADVANCED' : false
  const thirdPlaceCompleted = thirdPlaceMatch ? thirdPlaceMatch.status === 'COMPLETED' || thirdPlaceMatch.status === 'AUTO_ADVANCED' : false
...
async function recomputeAll(database: Database, bracketId: number, onlyIds?: Set<number>) {
  ...
  for (const m of ordered) {
    ...
    for (const s of mslots) {
      ...
      await database.update(bracketMatchSlots).set({ participantId: teamId }).where(eq(bracketMatchSlots.id, s.id))
    }
    ...
    await database.update(bracketMatches).set({ participant1Id: s1, participant2Id: s2, winnerId, loserId, status }).where(eq(bracketMatches.id, m.id))
...
export const tournamentService = {
  ...
  async generate(database: Database, input: GenerateInput) {
    ...
    if (existing && existing.status !== 'DRAFT') throw new Error('Bagan sudah dipublish. Reset/regenerate dulu untuk mengubah struktur.')
    ...
    return database.transaction(async (tx) => {
      ...
      await tx.insert(bracketParticipants).values(
        seedOrder.map((teamId, i) => ({ bracketId, teamId, seed: i + 1, initialSlot: i + 1, status: 'ACTIVE' as const })),
      )
      ...
      for (const r of structure.rounds) {
        const [row] = await tx
          ...
          .values({ bracketId, roundNumber: r.roundNumber, roundType: r.roundType, name: r.name, sortOrder: r.roundNumber })
          .returning()
      ...
      for (const m of structure.matches) {
        ...
        const s1Team = isFirst && m.slot1.kind === 'PARTICIPANT' ? slotTeams[(m.slot1 as { seedPosition: number }).seedPosition - 1] ?? null : null
        const s2Team = isFirst && m.slot2.kind === 'PARTICIPANT' ? slotTeams[(m.slot2 as { seedPosition: number }).seedPosition - 1] ?? null : null
        const [row] = await tx
          ...
          .returning()
        insertedMatches.push({ id: row.id, roundNumber: m.roundNumber, matchNumber: m.matchNumber })
      }

      const matchIdByRoundNumber = (rn: number, mn: number) => insertedMatches.find((x) => x.roundNumber === rn && x.matchNumber === mn)?.id ?? null
      ...
      for (const m of structure.matches) {
        ...
        const src = (slot: InitialSlot | DerivedSlot, slotIndex: number) => {
          if (slot.kind === 'PARTICIPANT') {
            ...
            if (tid === null) return { sourceType: 'BYE' as const, sourceTeamId: null, sourceMatchId: null }
            return { sourceType: 'PARTICIPANT' as const, sourceTeamId: tid, sourceMatchId: null }
          }
          if (slot.kind === 'BYE') return { sourceType: 'BYE' as const, sourceTeamId: null, sourceMatchId: null }
          ...
          return { sourceType: st, sourceTeamId: null, sourceMatchId: matchIdByRoundNumber(srcRound, slot.matchNumber) }
        }
        ...
        if (m.nextMatchNumber !== null && m.nextRoundNumber !== null) {
          await tx.update(bracketMatches).set({ nextMatchId: matchIdByRoundNumber(m.nextRoundNumber, m.nextMatchNumber), nextMatchSlot: m.nextSlot }).where(eq(bracketMatches.id, matchId))
        }
      }
      ...
      for (const m of structure.matches.filter((x) => x.roundNumber === 1)) {
        ...
        const [row] = await tx.select().from(bracketMatches).where(eq(bracketMatches.id, matchId)).limit(1)
        if (!row) continue
        const byeSlot = (row.participant1Id === null && row.participant2Id !== null) || (row.participant2Id === null && row.participant1Id !== null)
        if (byeSlot) {
          const winnerId = row.participant1Id ?? row.participant2Id
          await tx.update(bracketMatches).set({ winnerId, loserId: null, status: 'AUTO_ADVANCED' }).where(eq(bracketMatches.id, matchId))
          await tx.insert(bracketMatchResults).values({ matchId, winnerId: winnerId!, resultType: 'BYE' })
          await propagateWinner(tx, { id: matchId, winnerId, nextMatchId: row.nextMatchId, nextMatchSlot: row.nextMatchSlot })
        }
      }
  ...
  async submitResult(database: Database, input: SubmitResultInput) {
    return database.transaction(async (tx) => {
      const [match] = await tx.select().from(bracketMatches).where(eq(bracketMatches.id, input.matchId)).limit(1)
      ...
      if (match.status === 'COMPLETED' || match.status === 'AUTO_ADVANCED') throw new Error('Match sudah selesai. Gunakan koreksi hasil.')
      if (!canSubmitResult(match)) throw new Error('Match belum siap menerima hasil')
      if (!winnerBelongsToMatch(match, input.winnerId)) throw new Error('Pemenang harus peserta match ini')
      if (input.expectedVersion !== match.version) throw new Error('Data pertandingan sudah diubah admin lain. Silakan refresh.')

      // Idempotent guard tidak relevan (status COMPLETED ditolak di atas).

      const loserId = match.participant1Id === input.winnerId ? match.participant2Id : match.participant1Id
      ...
      await tx.update(bracketMatches).set({ winnerId: input.winnerId, loserId, status: 'COMPLETED', version: match.version + 1 }).where(eq(bracketMatches.id, match.id))
      ...
      const status = await computeBracketStatus(tx, match.bracketId, bracket.status, bracket.thirdPlaceEnabled)
      await tx.update(brackets).set({ status, updatedAt: new Date() }).where(eq(brackets.id, bracket.id))
  ...
  async correctResult(database: Database, input: CorrectResultInput) {
    return database.transaction(async (tx) => {
      const [match] = await tx.select().from(bracketMatches).where(eq(bracketMatches.id, input.matchId)).limit(1)
      if (!match) throw new Error('Match tidak ditemukan')
      if (match.status !== 'COMPLETED' && match.status !== 'AUTO_ADVANCED') throw new Error('Match belum selesai. Gunakan submit biasa.')
      ...
      if (affected > 0 && !input.invalidateDownstream) {
        throw new Error(`Koreksi ini membatalkan ${affected} hasil pertandingan berikutnya. Konfirmasi invalidate dulu.`)
      }
      ...
      if (downstream.size > 0) {
        await tx.delete(bracketMatchHistory).where(inArray(bracketMatchHistory.matchId, [...downstream]))
        await tx.delete(bracketMatchResults).where(inArray(bracketMatchResults.matchId, [...downstream]))
        for (const id of downstream) {
          await tx.update(bracketMatches).set({ participant1Id: null, participant2Id: null, winnerId: null, loserId: null, status: 'WAITING' }).where(eq(bracketMatches.id, id))
          const slots = await tx.select().from(bracketMatchSlots).where(eq(bracketMatchSlots.matchId, id))
          for (const s of slots) {
            await tx.update(bracketMatchSlots).set({ participantId: null }).where(eq(bracketMatchSlots.id, s.id))
          }
        }
  ...
  async publish(database: Database, bracketId: number) {
    ...
    if (!canEditStructure(b.status as BracketStatus)) throw new Error('Hanya bracket DRAFT yang bisa dipublish')
    await database.update(brackets).set({ status: 'PUBLISHED', updatedAt: new Date() }).where(eq(brackets.id, bracketId))
    return { ok: true }
  },
  ...
  async regenerate(database: Database, competitionId: number, kategori: 'putra' | 'putri', manualPositions?: Map<number, number>) {
    const b = await loadBracket(database, competitionId, kategori)
    if (!b) throw new Error('Bracket tidak ditemukan')
    if (!canEditStructure(b.status as BracketStatus)) throw new Error('Hanya bracket DRAFT yang bisa regenerate')
    const participants = await database.select().from(bracketParticipants).where(eq(bracketParticipants.bracketId, b.id)).orderBy(bracketParticipants.seed)
  ...
  async detail(database: Database, competitionId: number, kategori: 'putra' | 'putri'): Promise<BracketDetailView | null> {
    const b = await loadBracket(database, competitionId, kategori)
    if (!b) return null
    const rounds = await database.select().from(bracketRounds).where(eq(bracketRounds.bracketId, b.id)).orderBy(bracketRounds.sortOrder)
    const matches = await loadMatches(database, b.id)
    const participants = await database
      .select({ teamId: bracketParticipants.teamId, nama: teams.nama, seed: bracketParticipants.seed })
    ...
    const firstRoundMatches = firstRound
      ? matches.filter((m) => m.roundId === firstRound.id).sort((a, b) => a.matchNumber - b.matchNumber)
      : []
    ...
    const roundViews = rounds.map((r) => ({
      ...
      matches: matches
        ...
        .map((m) => ({
          ...
          participant1Nama: m.participant1Id !== null ? teamNama.get(String(m.participant1Id)) ?? null : null,
          participant2Nama: m.participant2Id !== null ? teamNama.get(String(m.participant2Id)) ?? null : null,
    ...
    return {
      ...
      thirdPlaceMatch: thirdPlaceMatch
        ? {
            ...
            participant1Nama: thirdPlaceMatch.participant1Id !== null ? teamNama.get(String(thirdPlaceMatch.participant1Id)) ?? null : null,
            participant2Nama: thirdPlaceMatch.participant2Id !== null ? teamNama.get(String(thirdPlaceMatch.participant2Id)) ?? null : null,
            seed1: thirdPlaceMatch.participant1Id !== null ? seedByTeam.get(thirdPlaceMatch.participant1Id) ?? null : null,
            seed2: thirdPlaceMatch.participant2Id !== null ? seedByTeam.get(thirdPlaceMatch.participant2Id) ?? null : null,
...
async function resetResultsFn(database: Database, bracketId: number) {
  await database.delete(bracketMatchHistory).where(sql`match_id in (select id from bracket_matches where bracket_id = ${bracketId})`)
  await database.delete(bracketMatchResults).where(sql`match_id in (select id from bracket_matches where bracket_id = ${bracketId})`)
  await recomputeAll(database, bracketId)
  await database.update(brackets).set({ status: 'PUBLISHED', updatedAt: new Date() }).where(eq(brackets.id, bracketId))
}

/** Kumpulkan id match downstream (via next-match chain + slot LOSER_OF), minus match sumber. */
async function collectDownstreamIds(database: Database, bracketId: number, fromMatchId: number): Promise<number[]> {
  ...
  const loserSlots = await database.select({ matchId: bracketMatchSlots.matchId }).from(bracketMatchSlots).where(and(eq(bracketMatchSlots.sourceType, 'LOSER_OF'), eq(bracketMatchSlots.sourceMatchId, fromMatchId)))
...
async function countPlayedDownstream(database: Database, bracketId: number, fromMatchId: number): Promise<number> {
