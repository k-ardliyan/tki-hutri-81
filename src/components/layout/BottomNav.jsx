...
export default function BottomNav() {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-50 p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] lg:hidden">
      <nav className="mx-auto grid max-w-lg grid-cols-4 gap-1 rounded-2xl border border-slate-200/90 bg-white/95 p-1.5 shadow-2xl shadow-slate-900/10 backdrop-blur-xl">
        {NAV.map((item) => {
          const active = activeId === item.id
          const loader = LOADERS[item.id]
          return (
            <RoutePrefetch key={item.id} loader={loader} className="flex min-w-0">
              <button
                type="button"
                onClick={() => {
