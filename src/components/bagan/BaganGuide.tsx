...
const STEPS: Record<BaganGuideStatus, Array<{ title: string; desc?: string }>> = {
  setup: [
    ...
    { title: 'Pilih seeding', desc: 'Acak = undian. Urutan Daftar = sesuai nomor. Manual = atur posisi sendiri.' },
    { title: 'Atur Perebutan Juara 3', desc: 'Aktif = kalah semifinal bertanding dulu untuk juara 3.' },
    { title: 'Klik Generate Bagan', desc: 'Struktur dibuat otomatis sesuai jumlah peserta. Masih draft, bisa diulang.' },
  ],
  draft: [
    { title: 'Cek panel Undian Seed', desc: 'Slot 1-2 = Match 1. S = urutan undian.' },
    { title: 'Tim dapat BYE otomatis maju', desc: 'Peserta ganjil membuat sebagian tim langsung lolos tanpa bertanding.' },
    { title: 'Kurang cocok? Acak Ulang', desc: 'Undian diganti. Bisa juga Hapus lalu generate ulang dengan seeding Manual.' },
  ...
  berlangsung: [
    ...
    { title: 'Salah input? Koreksi', desc: 'Jika match berikutnya sudah dimainkan, koreksi membatalkan hasil berikutnya.' },
  ],
  selesai: [
    { title: 'Podium otomatis', desc: 'Juara 1/2/3 dihitung dari hasil final dan perebutan juara 3.' },
    { title: 'Isi hadiah', desc: 'Panel Hadiah Juara per peringkat. Hadiah ikut tampil di podium.' },
  ],
}
...
export function BaganGuide({ status }: { status: BaganGuideStatus }) {
  ...
  return (
    <>
      <Button variant="outline" size="sm" className="rounded-xl font-bold" onClick={() => setOpen(true)}>
              ...
              return (
                <button
                  ...
                  className={`flex shrink-0 items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[11px] font-bold transition ${
                    active ? 'bg-brand-red text-white shadow-xs' : 'text-muted-foreground hover:bg-muted hover:text-foreground'
                  }`}
                >
                  <Icon size={12} />
                  {p.label}
                  {isCurrent && <span className={`h-1.5 w-1.5 rounded-full ${active ? 'bg-white/90' : 'bg-emerald-500'}`} />}
                </button>
              )
            })}
          </div>
            ...
            {steps.map((s, i) => (
              <li key={s.title} className="flex gap-3">
                ...
                <div className="min-w-0 pt-0.5">
                  ...
                  {s.desc && <p className="mt-0.5 text-xs leading-relaxed text-muted-foreground">{s.desc}</p>}
                </div>
              </li>
            ))}
          </ol>
