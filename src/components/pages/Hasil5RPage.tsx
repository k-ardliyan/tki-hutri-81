    ...
    <div className="space-y-5 sm:space-y-6">
      ...
      {!hasData && (
        <section className="surface-card p-8 text-center">
          ...
          <p className="mt-1 text-xs text-slate-400">Panitia akan mengisi setelah audit berlangsung.</p>
        </section>
      )}
      ...
      {hasData &&
        roomScores.map(({ room, count, final, scores, last }, rank) => {
          ...
          const scoreColor = final >= 80 ? 'text-status-done' : final >= 60 ? 'text-status-pending' : 'text-status-danger'
          return (
                ...
                <div className="flex items-center gap-3">
                  ...
                  {badge ? (
                    <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-black ${
                      badge === 'gold' ? 'bg-amber-100 text-amber-700' :
                      badge === 'silver' ? 'bg-slate-200 text-slate-600' :
              ...
              {scores.length > 0 && (
                <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-5">
                  {scores[0].categories.map((cat) => {
                    const barColor = cat.percent >= 80 ? 'bg-status-done' : cat.percent >= 60 ? 'bg-status-pending' : 'bg-status-danger'
                    return (
                      <div key={cat.categoryId} className="rounded-[var(--radius-md)] bg-slate-50 p-2.5">
                        <p className="text-[10px] font-bold text-slate-400">{cat.label}</p>
                        <p className="text-sm font-bold text-slate-800">
                          {round1(cat.percent)}<span className="text-xs text-slate-400">%</span>
                        </p>
