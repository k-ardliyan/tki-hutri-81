...
export default function SnackTeamAccordion({ teams, sessionId }: Props) {
  ...
  return (
    <Accordion type="single" collapsible value={openId ?? undefined} onValueChange={(v) => setOpenId(v ?? null)}>
      {teams.map((t) => {
          ...
          <AccordionItem key={t.id} value={String(t.id)} className="border-border">
            <AccordionTrigger className="px-4 hover:no-underline hover:bg-muted/40">
              <span className="flex min-w-0 items-center gap-2.5">
                <span className={`h-2.5 w-2.5 shrink-0 rounded-full ${t.done ? 'bg-success' : t.redeemed > 0 ? 'bg-warning' : 'bg-muted-foreground/30'}`} />
                <span className="min-w-0">
                  <span className="block truncate text-sm font-semibold text-foreground/90">{t.nama}</span>
