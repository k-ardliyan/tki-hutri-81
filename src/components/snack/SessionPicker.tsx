          ...
          <CommandList className="max-h-64">
            ...
            <CommandGroup>
              {filtered.map((s) => {
                ...
                return (
                  <CommandItem key={s.id} value={s.name} onSelect={() => pick(s)} className="flex items-center justify-between gap-2">
                    <span className="min-w-0 flex-1">
                      <span className={cn('block truncate font-semibold', isSelected && 'text-primary')}>{s.name}</span>
                    ...
                    {s.isActive && <Badge className="shrink-0 bg-success/10 text-success">AKTIF</Badge>}
