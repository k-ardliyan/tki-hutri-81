...
function AdminEmployees() {
  ...
  const columns = columnHelper.columns([
    columnHelper.accessor('nama', {
      ...
      enableHiding: false,
      cell: ({ row }) => (
        <div className="font-semibold text-foreground">{row.original.nama}</div>
      ),
    }),
      ...
      <ResponsiveDialog
        ...
        footer={
          <div className="flex w-full gap-2 sm:justify-end">
            <Button variant="outline" onClick={() => setShowDrawer(false)} className="flex-1 sm:flex-none">Batal</Button>
            <Button onClick={submit} className="flex-1 sm:flex-none">{editId !== null ? 'Simpan' : 'Tambah'}</Button>
          </div>
        }
      >
