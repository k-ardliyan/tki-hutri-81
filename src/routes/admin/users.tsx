...
function SuperadminUsers() {
  ...
  const toggleActive = async (u: UserRow) => {
    await updateUser({ data: { id: u.id, isActive: !u.isActive } });
    await load();
    toast.success(u.isActive ? 'User dinonaktifkan' : 'User diaktifkan');
  };
  const changeRole = async (u: UserRow, next: UserRole) => {
    await updateUser({ data: { id: u.id, role: next } });
    await load();
  };
  ...
  const remove = async () => {
    ...
    await deleteUser({ data: { id: deleteTarget.id } });
    ...
    columnHelper.accessor('employeeNama', {
      ...
      cell: ({ row }) => (
        <span className="text-muted-foreground">
          {row.original.employeeNama ?? '(tanpa karyawan)'}
        </span>
      ),
    }),
