  ...
  const columns = columnHelper.columns([
    columnHelper.accessor('name', {
      ...
      cell: ({ row }) => (
        <div>
          ...
          <p className="text-xs text-muted-foreground">#{row.original.id} · Kuota {row.original.quota}</p>
        </div>
      ),
    }),
    ...
    columnHelper.accessor('remaining', {
      ...
      cell: ({ row }) => {
        ...
        const pct = s.quota > 0 ? Math.max(0, Math.min(100, ((s.quota - s.remaining) / s.quota) * 100)) : 0
        const tone = s.remaining === 0 ? 'bg-destructive' : s.remaining <= s.quota * 0.2 ? 'bg-warning' : 'bg-success'
        return (
          <div className="flex w-36 items-center gap-2">
            <Progress value={pct} className={`h-1.5 [&_[data-slot=progress-indicator]]:${tone}`} />
            <span className={`shrink-0 text-xs font-bold font-mono ${s.remaining === 0 ? 'text-destructive' : s.remaining <= s.quota * 0.2 ? 'text-warning' : 'text-success'}`}>
              {s.remaining}
            </span>
          </div>
        )
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-44">
              <DropdownMenuItem onClick={() => { setEditTarget(s); setEditName(s.name); setEditQuota(String(s.quota)) }}>
                <UserPen />Edit Sesi
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => toggleActive(s)}>
                {s.isActive ? <PowerOff /> : <Power />}{s.isActive ? 'Nonaktifkan' : 'Aktifkan'}
              </DropdownMenuItem>
              <DropdownMenuSeparator />
    ...
    return (
      <div className="space-y-6">
        <PageHeader
          ...
          action={<Button disabled><Plus size={14} className="mr-1.5" />Tambah Sesi</Button>}
        />
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        ...
        toolbar={<span className="text-sm font-medium text-muted-foreground">{sessions.length} sesi terdaftar</span>}
      />
      ...
      <ResponsiveDialog
        ...
        footer={
          <div className="flex w-full gap-2 sm:justify-end">
            ...
            <Button onClick={() => void doCreate()} className="flex-1 sm:flex-none">Simpan</Button>
          </div>
        }
      >
      ...
      <ResponsiveDialog
        ...
        footer={
          <div className="flex w-full gap-2 sm:justify-end">
            ...
            <Button onClick={() => void doEdit()} className="flex-1 sm:flex-none">Simpan</Button>
          </div>
        }
      >
        <div className="space-y-3">
