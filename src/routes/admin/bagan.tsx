...
import { BracketTree, BracketTreeSkeleton, type BracketDetailView, type MatchView } from '../../components/bagan/BracketTree'
  ...
  return (
    <div className="flex w-full items-center gap-1.5">
      <Input
        ...
        aria-label="Hadiah"
      />
      <Button size="sm" variant="outline" className="h-8 px-2 text-xs font-bold" disabled={!changed || busy} onClick={() => void save()}>
        Simpan
      </Button>
    </div>
  )
...
function AdminBagan() {
  const [comps, setComps] = useState<Array<{ id: number; slug: string; short: string; title: string }>>([])
  const [teamsByKategori, setTeamsByKategori] = useState<Record<Kategori, Array<{ id: number; nama: string }>>>({ putra: [], putri: [] })
  ...
  // Confirm dialogs
  const [confirmAction, setConfirmAction] = useState<{ key: string; title: string; desc: string; run: () => Promise<void> } | null>(null)
  ...
  const loadTab = useCallback(async (compId: number, kategori: Kategori) => {
    ...
    setTabData((prev) => ({ ...prev, [key]: { ...(prev[key] ?? { detail: null, prizes: [], loading: true }), loading: true } }))
    try {
      ...
      setTabData((prev) => ({ ...prev, [key]: { ...(prev[key] ?? { detail: null, prizes: [], loading: false }), loading: false } }))
    }
  }, [])
  ...
  const manualValid = useMemo(() => {
    ...
    if (vals.some((v) => !Number.isInteger(v) || v! < 1 || v! > n)) return { ok: false, msg: `Posisi harus angka 1 sampai ${n}` }
  ...
  const submitCorrect = async () => {
    ...
    try {
      ...
      toast.success(res.invalidated > 0 ? `Koreksi disimpan. ${res.invalidated} hasil berikutnya dibatalkan.` : 'Koreksi disimpan')
  ...
  const doGenerate = async () => {
    setBusy('generate')
    try {
      const manual = setupSeeding === 'MANUAL' ? Object.fromEntries(Object.entries(manualPositions[kategori] ?? {}).map(([k, v]) => [k, v])) : undefined
      await generateBracket({
        data: { competitionId: comp.id, kategori, seedingMethod: setupSeeding, thirdPlaceEnabled: setupThirdPlace, manualPositions: manual },
      })
  ...
  const matchCount = detail ? detail.rounds.filter((r) => r.roundType === 'MAIN').reduce((a, r) => a + r.matches.length, 0) + (detail.thirdPlaceMatch ? 1 : 0) : 0
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <PageHeader title="Bagan Pertandingan" subtitle="Atur bagan gugur, input hasil, dan kelola hadiah juara." />
            ...
            return (
              <button
                ...
                className={`flex shrink-0 items-center gap-1.5 rounded-xl px-3.5 py-2 text-xs font-bold transition ${
                  activeKey === key ? 'bg-brand-red text-white shadow-xs' : 'text-muted-foreground hover:bg-muted hover:text-foreground'
                }`}
              >
                {label}
                {hasBracket && <CheckCircle2 size={12} className={activeKey === key ? 'text-white' : 'text-emerald-500'} />}
              </button>
            )
          }),
        )}
        ...
        <Card className="rounded-3xl border border-border bg-card shadow-xs">
          <CardContent className="space-y-4 p-5 sm:p-6">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-4">
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  {comp.title} · {kategori === 'putra' ? 'Putra' : 'Putri'} · {teamsByKategori[kategori].length} peserta terdaftar
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-1.5">
                <span className="text-xs font-bold uppercase text-muted-foreground">Perebutan Juara 3</span>
                <Select value={setupThirdPlace ? 'MATCH' : 'NONE'} onValueChange={(v) => setSetupThirdPlace(v === 'MATCH')}>
                  <SelectTrigger className="w-full rounded-xl">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="mt-2 grid gap-x-6 gap-y-1 sm:grid-cols-2">
                {teamsByKategori[kategori].map((t, i) => (
                  <div key={t.id} className="flex items-center gap-2 text-xs">
                    <span className="w-5 shrink-0 text-right font-black text-muted-foreground">{i + 1}.</span>
            ...
            {setupSeeding === 'MANUAL' && (
              <div className="space-y-2 rounded-xl border border-border bg-muted/20 p-3">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Posisi Seed</p>
                  <span className="text-[10px] text-muted-foreground">1 = match pertama</span>
                </div>
                <div className="grid gap-1.5 sm:grid-cols-2">
                  {teamsByKategori[kategori].map((t, i) => (
                    <div key={t.id} className="flex items-center gap-2">
                      <span className="w-6 shrink-0 text-right text-[11px] font-black text-muted-foreground">{i + 1}.</span>
                      <span className="min-w-0 flex-1 truncate text-xs font-bold text-foreground">{t.nama}</span>
                      <Input
                        type="number"
                        min={1}
                ...
                {!manualValid.ok && (
                  <p className="rounded-lg bg-destructive/10 px-2.5 py-1.5 text-[11px] font-bold text-destructive">{manualValid.msg}</p>
                )}
              </div>
            )}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border pt-4">
              <p className="text-[11px] text-muted-foreground">
                Struktur dibuat otomatis: bracket {Math.max(2, 2 ** Math.ceil(Math.log2(Math.max(2, teamsByKategori[kategori].length))))} slot.
                Tim yang dapat BYE otomatis maju.
              </p>
              <Button
                onClick={() => void run('generate', doGenerate)}
                disabled={busy !== null || teamsByKategori[kategori].length < 2 || (setupSeeding === 'MANUAL' && !manualValid.ok)}
          ...
          <Card className="rounded-3xl border border-border bg-card shadow-xs">
            <CardContent className="space-y-4 p-5 sm:p-6">
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-4">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="font-heading text-base font-black text-foreground">{comp.title} · {kategori === 'putra' ? 'Putra' : 'Putri'}</h3>
                    <span className={`rounded-full border px-2.5 py-0.5 text-[10px] font-black uppercase tracking-wider ${STATUS_STYLE[detail.bracket.status] ?? STATUS_STYLE.DRAFT}`}>
                      {STATUS_LABEL[detail.bracket.status] ?? detail.bracket.status}
                    </span>
                  </div>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {detail.bracket.participantCount} peserta · {matchCount} match · {roundCount} round · seeding {detail.bracket.seedingMethod === 'RANDOM' ? 'acak' : detail.bracket.seedingMethod === 'MANUAL' ? 'manual' : 'urutan daftar'}
                    {detail.bracket.thirdPlaceEnabled ? ' · perebutan juara 3 aktif' : ''}
                  </p>
                </div>
                <div className="flex flex-wrap gap-2">
                  {isDraft && (
                    <Button size="sm" className="rounded-xl font-bold" onClick={() => void run('publish', doPublish)} disabled={busy !== null}>
                      Publish Bagan
                    </Button>
                  )}
                  {isDraft && (
                    <Button size="sm" variant="outline" className="rounded-xl font-bold" onClick={() => void run('regenerate', async () => {
                  ...
                  {!isDraft && (
                    <Button size="sm" variant="outline" className="rounded-xl font-bold" onClick={() => setConfirmAction({
                      key: 'reset',
                      title: 'Reset hasil?',
                  ...
                  <Button size="sm" variant="destructive" className="rounded-xl font-bold" onClick={() => setConfirmAction({
                    key: 'delete',
                    title: 'Hapus bagan?',
              ...
              {detail.slots.length > 0 && (
                <div className="rounded-xl border border-border bg-muted/20 p-3">
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Undian Seed</p>
                    <span className="text-[10px] text-muted-foreground">Slot 1-2 = Match 1 · S = urutan undian</span>
                  </div>
                  <div className="mt-2 grid gap-x-6 gap-y-1 sm:grid-cols-2">
                    {detail.slots.map((s) => (
                      <div key={s.slot} className="flex items-center gap-2 text-xs">
                        <span className="w-10 shrink-0 text-right font-black text-muted-foreground">Slot {s.slot}</span>
                        {s.bye ? (
                          <span className="rounded-md bg-muted px-1.5 py-0.5 text-[10px] font-bold text-muted-foreground">BYE</span>
                        ) : (
                          <>
                            {s.seed !== null && (
                              <span className="shrink-0 rounded bg-brand-red/10 px-1 text-[9px] font-black leading-4 text-brand-red">S{s.seed}</span>
                            )}
            ...
            <CardContent className="space-y-3 p-5 sm:p-6">
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Podium dihitung otomatis dari hasil match. Hadiah mengikuti peringkat.</p>
                </div>
                <Button
                  size="sm"
                  variant="outline"
                  className="rounded-xl font-bold"
                  onClick={() => void run('addPrize', async () => {
                    const next = (prizes.reduce((m, p) => Math.max(m, p.place), 0) ?? 0) + 1
                    await upsertPrize({ data: { competitionId: comp.id, kategori, place: next, hadiah: '' } })
              ...
              {prizes.length === 0 ? (
                ...
                <div className="space-y-2">
                  {prizes.map((p) => {
                    const teamId = p.place === 1 ? detail.podium.rank1 : p.place === 2 ? detail.podium.rank2 : p.place === 3 ? detail.podium.rank3 : null
                    const winnerNama = teamId !== null ? detail.participants.find((x) => x.teamId === teamId)?.nama ?? null : null
                    const emoji = p.place === 1 ? '🥇' : p.place === 2 ? '🥈' : p.place === 3 ? '🥉' : `#${p.place}`
                    return (
                      <div key={p.place} className="flex flex-wrap items-center gap-2 rounded-xl border border-border bg-muted/20 px-3 py-2">
                        <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-muted text-xs font-black">{emoji}</span>
                        <div className="min-w-0 flex-1 basis-40">
                          <p className="text-xs font-black text-foreground">
                            Juara {p.place}
                            {winnerNama && <span className="ml-1.5 font-extrabold text-emerald-600 dark:text-emerald-400">- {winnerNama}</span>}
                          </p>
                          <HadiahInput
                            key={`h-${comp.id}-${kategori}-${p.place}`}
                            initial={p.hadiah}
                            onSave={async (value) => {
                              await upsertPrize({ data: { competitionId: comp.id, kategori, place: p.place, hadiah: value } })
                        ...
                        <Button
                          ...
                          onClick={() => void run(`delPrize-${p.place}`, async () => {
                            await deletePrize({ data: { competitionId: comp.id, kategori, place: p.place } })
          ...
          <div className="space-y-4 py-2">
              ...
              <div className="space-y-2">
                {[
                  ...
                  p.id !== null ? (
                    ...
                    >
                      <span className={`flex h-4 w-4 items-center justify-center rounded-full border ${resultWinner === p.id ? 'border-brand-red bg-brand-red' : 'border-muted-foreground/40'}`}>
                        {resultWinner === p.id && <span className="h-1.5 w-1.5 rounded-full bg-white" />}
                      </span>
                      {p.nama}
                      <span className="ml-auto text-[10px] font-bold text-muted-foreground">Tim {i + 1}</span>
                    </button>
                  ) : null,
                )}
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <span className="text-xs font-bold uppercase text-muted-foreground">Skor Tim 1</span>
                <Input type="number" min={0} value={resultScore1} onChange={(e) => setResultScore1(e.target.value)} aria-label="Skor tim 1" className="h-9 rounded-lg" />
              </div>
              <div className="space-y-1">
                <span className="text-xs font-bold uppercase text-muted-foreground">Skor Tim 2</span>
                <Input type="number" min={0} value={resultScore2} onChange={(e) => setResultScore2(e.target.value)} aria-label="Skor tim 2" className="h-9 rounded-lg" />
              </div>
            </div>

            <div className="space-y-1">
              <span className="text-xs font-bold uppercase text-muted-foreground">Tipe Hasil</span>
              <Select value={resultType} onValueChange={(v) => setResultType(v as typeof resultType)}>
                <SelectTrigger className="w-full rounded-xl">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-1">
              <span className="text-xs font-bold uppercase text-muted-foreground">Catatan (opsional)</span>
              <Input type="text" value={resultNotes} onChange={(e) => setResultNotes(e.target.value)} placeholder="Contoh: adu penalti" className="h-9 rounded-lg" aria-label="Catatan hasil" />
            </div>
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button size="sm" onClick={() => void submitResult()} disabled={busy === 'result' || resultWinner === null}>
      ...
      <ResponsiveDialog
        ...
        title="Koreksi Hasil"
        description={correctTarget ? `Match ${correctTarget.matchNumber} sudah selesai. Ubah pemenang dengan hati-hati.` : ''}
      >
        {correctTarget && (
          <div className="space-y-4 py-2">
            {correctBlockMsg && (
            ...
            <div className="space-y-1.5">
              <span className="text-xs font-bold uppercase text-muted-foreground">Pemenang Baru</span>
              <div className="space-y-2">
                {[
                  ...
                  p.id !== null ? (
                    ...
                    >
                      <span className={`flex h-4 w-4 items-center justify-center rounded-full border ${correctWinner === p.id ? 'border-brand-red bg-brand-red' : 'border-muted-foreground/40'}`}>
                        {correctWinner === p.id && <span className="h-1.5 w-1.5 rounded-full bg-white" />}
                      </span>
                      {p.nama}
                      {p.id === correctTarget.winnerId && <span className="ml-auto text-[10px] font-bold text-muted-foreground">pemenang saat ini</span>}
                    </button>
                  ) : null,
                )}
              </div>
            ...
            <div className="space-y-1">
              <span className="text-xs font-bold uppercase text-muted-foreground">Alasan (opsional)</span>
              <Input type="text" value={correctReason} onChange={(e) => setCorrectReason(e.target.value)} placeholder="Contoh: salah input" className="h-9 rounded-lg" aria-label="Alasan koreksi" />
            </div>
            {correctInvalidate && (
              <label className="flex items-start gap-2 rounded-xl border border-amber-500/40 bg-amber-500/5 px-3 py-2 text-[11px] font-bold text-amber-800 dark:text-amber-300">
                <input type="checkbox" checked={correctInvalidate} onChange={(e) => setCorrectInvalidate(e.target.checked)} className="mt-0.5" />
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button size="sm" variant={correctInvalidate ? 'destructive' : 'default'} onClick={() => void submitCorrect()} disabled={busy === 'correct' || correctWinner === null}>
