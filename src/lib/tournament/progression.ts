...
export function getMatchDestination(match: TournamentMatch): { matchId: number; slot: 1 | 2 } | null {
...
export function getLoserDestination(match: TournamentMatch): { matchId: number; slot: 1 | 2 } | null {
...
export function resolveDownstreamStatus(
  ...
  if (match.status === 'COMPLETED' || match.status === 'AUTO_ADVANCED' || match.status === 'CANCELLED') {
    return match.status
  }
  if (match.participant1Id !== null && match.participant2Id !== null) return 'READY'
  if ((match.participant1Id !== null && slot2IsBye) || (match.participant2Id !== null && slot1IsBye)) return 'READY'
  return 'WAITING'
}
...
export function isRoundCompleted(matches: TournamentMatch[]): boolean {
  return matches.length > 0 && matches.every((m) => m.status === 'COMPLETED' || m.status === 'AUTO_ADVANCED')
}
