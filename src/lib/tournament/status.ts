...
export function nextBracketStatus(
  ...
  event: { published: boolean; hasAnyResult: boolean; finalCompleted: boolean; thirdPlaceEnabled: boolean; thirdPlaceCompleted: boolean },
): BracketStatus {
  ...
  if (isBracketCompleted(event.finalCompleted, event.thirdPlaceEnabled, event.thirdPlaceCompleted)) return 'COMPLETED'
