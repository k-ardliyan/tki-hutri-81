...
check(
  `M1 = ${P1} vs BYE (AUTO)`,
  qf[0].participant1Id === P1 &&
    qf[0].participant2Id === null &&
    qf[0].status === 'AUTO_ADVANCED' &&
    qf[0].winnerId === P1
);
check(
  `M2 = ${P3} vs ${P4} (READY)`,
  qf[1].participant1Id === P3 && qf[1].participant2Id === P4 && qf[1].status === 'READY'
);
check(
  `M3 = ${P5} vs BYE (AUTO)`,
  qf[2].participant1Id === P5 &&
    qf[2].participant2Id === null &&
    qf[2].status === 'AUTO_ADVANCED' &&
    qf[2].winnerId === P5
);
check(
  `M4 = ${P2} vs BYE (AUTO, shift)`,
  qf[3].participant1Id === P2 &&
    qf[3].participant2Id === null &&
    qf[3].status === 'AUTO_ADVANCED' &&
    qf[3].winnerId === P2
);
const sf = d!.rounds[1].matches;
check(
  `SF1 = ${P1} vs menunggu M2 (WAITING)`,
  sf[0].participant1Id === P1 && sf[0].participant2Id === null && sf[0].status === 'WAITING'
);
check(
  `SF2 = ${P5} vs ${P2} (READY, BYE advance)`,
  sf[1].participant1Id === P5 && sf[1].participant2Id === P2 && sf[1].status === 'READY'
);
check(
  'slots BYE shift (seed 2 pindah ke slot 7)',
  JSON.stringify(d!.slots.map((s) => (s.bye ? 'B' : String(s.seed)))) ===
    '["1","B","3","4","5","B","2","B"]'
);
