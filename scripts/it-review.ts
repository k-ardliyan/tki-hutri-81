...
const putraIds = (await (db!).select({ id: teams.id }).from(teams).where(eq(teams.kategori, 'putra')).orderBy(teams.nomor).limit(8)).map((t) => t.id)
...
const fresh = async (kategori: 'putra' | 'putri', teamIds: number[], thirdPlaceEnabled = true) => {
  ...
  const { bracketId } = await tournamentService.generate(td, { competitionId: comp!.id, kategori, teamIds, seedingMethod: 'REGISTRATION_ORDER', thirdPlaceEnabled })
  bracketIds.push(bracketId)
  return { competitionId: comp!.id, bracketId }
}
const submit = async (competitionId: number, kategori: 'putra' | 'putri', matchId: number, winnerId: number, extra: Record<string, unknown> = {}) => {
  const d = await tournamentService.detail(td, competitionId, kategori)
  const m = d!.rounds.flatMap((r) => r.matches).find((x) => x.id === matchId) ?? (d!.thirdPlaceMatch?.id === matchId ? d!.thirdPlaceMatch : null)
  return tournamentService.submitResult(td, { matchId, winnerId, expectedVersion: m!.version, ...extra })
}
...
console.log('koreksi tanpa invalidate (downstream WAITING)')
{
  ...
  check('third place slot1 ikut berubah = A (loser baru SF1)', d!.thirdPlaceMatch!.participant1Id === A)
  check('third place slot2 masih kosong (SF2 belum selesai)', d!.thirdPlaceMatch!.participant2Id === null)
  check('SF1 winner = C', d!.rounds[1].matches[0].winnerId === C)
  check('QF tetap COMPLETED', d!.rounds[0].matches.every((m) => m.status === 'COMPLETED'))
}
...
console.log('walkover / lock / draft block')
{
  ...
  try {
    await tournamentService.submitResult(td, { matchId: qf[0].id, winnerId: A, expectedVersion: 99 })
  } catch (e) {
  ...
  await tournamentService.submitResult(td, { matchId: qf[0].id, winnerId: B, resultType: 'WALKOVER', expectedVersion: 1 })
  d = await tournamentService.detail(td, competitionId, 'putra')
  check('walkover tersimpan (winner B, WALKOVER)', (() => {
...
console.log('reset results')
{
  ...
  await tournamentService.publish(td, bracketId)
  let d = await tournamentService.detail(td, competitionId, 'putra')
  for (const m of d!.rounds[0].matches) await submit(competitionId, 'putra', m.id, m.participant1Id!)
  d = await tournamentService.detail(td, competitionId, 'putra')
  for (const m of d!.rounds[1].matches) await submit(competitionId, 'putra', m.id, m.participant1Id!)
  d = await tournamentService.detail(td, competitionId, 'putra')
  await submit(competitionId, 'putra', d!.rounds[2].matches[0].id, d!.rounds[2].matches[0].participant1Id!)
  ...
  check('QF kembali READY', d!.rounds[0].matches.every((m) => m.status === 'READY'))
  check('SF kembali WAITING', d!.rounds[1].matches.every((m) => m.status === 'WAITING'))
...
console.log('regenerate draft')
{
  ...
  check('peserta sama (8)', after!.bracket.participantCount === 8 && after!.bracket.participantCount === before!.bracket.participantCount)
...
console.log('16 peserta (4 round, 15 match)')
{
  ...
  if (teamRows.length < 16) {
    ...
    check('R1 8 match READY', main[0].matches.length === 8 && main[0].matches.every((m) => m.status === 'READY'))
    check('R4 = Final 1 match', main[3].name === 'Final' && main[3].matches.length === 1)
    check('M1 destination R2 M1 slot1', main[0].matches[0].nextMatchId === main[1].matches[0].id && main[0].matches[0].nextMatchSlot === 1)
    check('M2 destination R2 M1 slot2', main[0].matches[1].nextMatchId === main[1].matches[0].id && main[0].matches[1].nextMatchSlot === 2)
  }
}
...
console.log('third place disabled')
{
  ...
  let d = await tournamentService.detail(td, competitionId, 'putra')
  check('tanpa round third', !d!.rounds.some((r) => r.roundType === 'THIRD_PLACE') && d!.thirdPlaceMatch === null)
  await tournamentService.publish(td, bracketId)
  ...
  check(`podium ${A}/${E}/null`, JSON.stringify(d!.podium) === `{"rank1":${A},"rank2":${E},"rank3":null}`)
}
...
console.log('manual seeding')
{
  ...
  const { competitionId, bracketId } = await fresh('putra', T8)
  const manual = new Map<number, number>([[A, 2], [B, 1], [C, 3], [D, 4], [E, 5], [F, 6], [G, 7], [H, 8]])
  const { bracketId: manualBracketId } = await tournamentService.generate(td, { competitionId, kategori: 'putra', teamIds: T8, seedingMethod: 'MANUAL', thirdPlaceEnabled: true, manualPositions: manual })
  bracketIds.push(manualBracketId)
  let d = await tournamentService.detail(td, competitionId, 'putra')
  const qf = d!.rounds[0].matches
  check(`M1 = ${B} vs ${A} (posisi ditukar)`, qf[0].participant1Id === B && qf[0].participant2Id === A)
  ...
  check('slots mencerminkan swap', JSON.stringify(d!.slots.map((s) => s.teamId)) === JSON.stringify([B, A, C, D, E, F, G, H]))
  // Hapus dulu (fresh sudah bikin bracket lain), generate MANUAL dobel posisi → throw.
  await tournamentService.remove(td, bracketId)
  const dup = new Map<number, number>([[A, 1], [B, 1], [C, 3], [D, 4], [E, 5], [F, 6], [G, 7], [H, 8]])
  let dupBlocked = false
  try {
    await tournamentService.generate(td, { competitionId, kategori: 'putra', teamIds: T8, seedingMethod: 'MANUAL', thirdPlaceEnabled: true, manualPositions: dup })
  } catch (e) {
    dupBlocked = e instanceof Error && e.message.includes('posisi seed')
  }
  check('MANUAL posisi dobel → ditolak', dupBlocked)
  const miss = new Map<number, number>([[A, 1], [C, 3], [D, 4], [E, 5], [F, 6], [G, 7], [H, 8]])
  let missBlocked = false
  try {
    await tournamentService.generate(td, { competitionId, kategori: 'putra', teamIds: T8, seedingMethod: 'MANUAL', thirdPlaceEnabled: true, manualPositions: miss })
  } catch (e) {
