...
import {
  ...
  calculateRoundCount,
  nextPowerOfTwo,
} from '../src/lib/tournament/bracket-size';
...
check(
  'third place ada',
  g8.rounds.some((r) => r.roundType === 'THIRD_PLACE')
);
check(
  'M1 winner → SF match 1 slot 1',
  (() => {
...
check(
  ...
  (() => {
    ...
    return (
      tp.slot1.kind === 'LOSER_OF' &&
      tp.slot2.kind === 'LOSER_OF' &&
      (tp.slot1 as { matchNumber: number }).matchNumber === 1 &&
      (tp.slot2 as { matchNumber: number }).matchNumber === 2
    );
  })()
);
...
check(
  'slot 2 M4 = BYE (posisi 8)',
  (() => {
    const m4 = g5.matches.find((m) => m.matchNumber === 4 && m.roundNumber === 1)!;
    return (
      m4.slot2.kind === 'BYE' ||
      ((m4.slot2 as { seedPosition: number }).seedPosition === 8 && g5.byeCount === 3)
    );
  })()
);
...
check(
  'REGISTRATION_ORDER = urutan',
  JSON.stringify(generateSeedOrder('REGISTRATION_ORDER', parts)) === '[1,2,3,4,5]'
);
...
check(
  'MANUAL urut sesuai posisi',
  JSON.stringify(generateSeedOrder('MANUAL', parts, manual)) === '[1,2,3,4,5]'
);
const manualSwap = new Map<number, number>([
  [1, 5],
  [2, 1],
  [3, 2],
  [4, 3],
...
check(
  'MANUAL acak posisi',
  JSON.stringify(generateSeedOrder('MANUAL', parts, manualSwap)) === '[2,3,4,5,1]'
);
...
const missPos = new Map<number, number>([
  [1, 1],
  [2, 2],
  [3, 3],
  [4, 4],
...
check(
  'canSubmitResult COMPLETED ditolak',
  !canSubmitResult(mkMatch({ participant1Id: 1, participant2Id: 2, status: 'COMPLETED' }))
);
check('winner belongs to match', winnerBelongsToMatch(ready, 2));
check('winner asing ditolak', !winnerBelongsToMatch(ready, 99));
check(
  'edit struktur hanya DRAFT',
  canEditStructure('DRAFT') && !canEditStructure('PUBLISHED') && !canEditStructure('IN_PROGRESS')
);
...
check(
  'final selesai tapi third belum → belum COMPLETED',
  nextBracketStatus('IN_PROGRESS', {
    published: false,
    hasAnyResult: true,
    finalCompleted: true,
...
check(
  'podium tanpa third',
  JSON.stringify(calculatePodium(finalDone, null)) === '{"rank1":1,"rank2":2,"rank3":null}'
);
check(
  'final belum selesai → kosong',
  JSON.stringify(calculatePodium(mkMatch({ participant1Id: 1, participant2Id: 2 }), null)) ===
