...
import { competitionSectionsSeed, competitionsSeed } from './seed-data/competitions';
import { employeesSeed } from './seed-data/employees';
import {
  eventsSeed,
  keyDatesSeed,
  landingHighlightsSeed,
  roomAreasSeed,
  rundownItemsSeed,
  rundownPhasesSeed,
} from './seed-data/events';
...
async function main() {
  ...
  const insertedEmployees = await db
    .insert(employees)
    .values(employeesSeed)
    .returning({ id: employees.id, nama: employees.nama });
  ...
  for (const team of teamsSeed) {
    const [inserted] = await db
      .insert(teams)
      .values(team)
      .returning({ id: teams.id, kode: teams.kode });
  ...
  for (let i = 0; i < rundownPhasesSeed.length; i++) {
    ...
    const phaseNum = i + 1;
    const [inserted] = await db
      .insert(rundownPhases)
      .values(phase)
      .returning({ id: rundownPhases.id });
  ...
  for (let i = 0; i < competitionsSeed.length; i++) {
    ...
    const [inserted] = await db
      .insert(competitions)
      .values(comp)
      .returning({ id: competitions.id, slug: competitions.slug });
  ...
  if (dekorCompId) {
    await db
      ...
      .values({
        competitionId: dekorCompId,
        deadline: null,
        note: 'Tenggat penilaian lomba dekorasi & 5R (di-set admin)',
      })
      .onConflictDoNothing();
  }
  ...
  console.log(
    `   - ${teamsSeed.length} teams + ${memberCount} team_members${unmatched.length ? ` (⚠️ ${unmatched.length} unmatched)` : ''}`
  );
  console.log(
    `   - ${superadminNames.length + auditNames.length} users (3 superadmin + 6 audit, password: admin123)`
  );
