...
import { scoreSubmission, combineFinal } from '../src/lib/scoring'
...
if (dekor) {
  ...
  check('max kategori 20/25/25/15/15', JSON.stringify(maxes) === JSON.stringify([20, 25, 25, 15, 15]), JSON.stringify(maxes))
  check('order kriteria 1..20', dekor.categories.flatMap((c) => c.criteria.map((k) => k.order)).join(',') === Array.from({ length: 20 }, (_, i) => i + 1).join(','))
  ...
  const sub = (answers: Record<string, number>) => ({
    id: 't', roomId: 'sales', formId: 'dekorasi', auditor: 'A', answers,
    notes: {}, submittedAt: new Date().toISOString(), createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
  })

  const all5 = scoreSubmission(dekor, sub(Object.fromEntries(dekor.categories.flatMap((c) => c.criteria.map((k) => [k.id, 5])))))
  check('semua 5 → final 100', all5.final === 100, `final=${all5.final}`)

  const all3 = scoreSubmission(dekor, sub(Object.fromEntries(dekor.categories.flatMap((c) => c.criteria.map((k) => [k.id, 3])))))
  ...
  for (const c of dekor.categories) {
    const val = c.id === 'tema' ? 4 : c.id === 'kreativitas' ? 3 : c.id === 'keindahan' ? 5 : c.id === 'area' ? 2 : 4
    for (const k of c.criteria) mixed[k.id] = val
  }
  const m = scoreSubmission(dekor, sub(mixed))
  check('campuran → final 74 (weighted, bukan mean-percents 72)', m.final === 74 && m.totalRaw === 74, `final=${m.final} raw=${m.totalRaw}`)
}

// ── Form 5R lama (uniform) tidak berubah ──
const ns = getFiveRForm('office-non-smoking')
if (ns) {
  const sub = { id: 't', roomId: 'sales', formId: ns.id, auditor: 'A', answers: {} as Record<string, number>, notes: {}, submittedAt: new Date().toISOString(), createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() }
  ...
  check('5R uniform: final === totalRaw/totalMax (max sama tiap kategori)', Math.abs(s.final - (s.totalRaw / s.totalMax) * 100) < 1e-9, `final=${s.final} raw=${s.totalRaw}/${s.totalMax}`)
}

// ── combineFinal (70/30) ──
check('combineFinal(80, 60) = 74', combineFinal(80, 60) === 74, `=${combineFinal(80, 60)}`)
check('combineFinal(0, 100) = 30', Math.abs(combineFinal(0, 100) - 30) < 1e-9, `=${combineFinal(0, 100)}`)
...
check('isDekorasiSubmission(office-smoking) false', isDekorasiSubmission('office-smoking') === false)
