...
async function cleanup() {
  if (teamAId) await (db!).delete(teams).where(eq(teams.id, teamAId)).catch(() => {})
  if (teamBId) await (db!).delete(teams).where(eq(teams.id, teamBId)).catch(() => {})
  if (empId) await (db!).delete(employees).where(eq(employees.id, empId)).catch(() => {})
  // sisa aman (kalau delete gagal karena FK)
  await (db!).delete(teamMembers).where(inArray(teamMembers.teamId, [teamAId, teamBId])).catch(() => {})
}

// ── setup: 2 tim + 1 karyawan ──
const [a] = await (db!).insert(teams).values({ kategori: 'putra', nomor: 99, nama: 'IT Test A', kode: TEST_TEAM_CODES[0] }).returning()
teamAId = a.id
const [b] = await (db!).insert(teams).values({ kategori: 'putri', nomor: 99, nama: 'IT Test B', kode: TEST_TEAM_CODES[1] }).returning()
teamBId = b.id
const [emp] = await (db!).insert(employees).values({ nama: 'IT Test Karyawan', nip: TEST_NIP }).returning()
empId = emp.id
...
let dupKode = false
try {
  await (db!).insert(teams).values({ kategori: 'putra', nomor: 100, nama: 'IT Test Dupe', kode: TEST_TEAM_CODES[0] })
} catch (e) {
...
const afterTeamDelete = await (db!).select({ id: teamMembers.id }).from(teamMembers).where(eq(teamMembers.teamId, teamAId))
...
const afterEmpDelete = await (db!).select({ id: teamMembers.id }).from(teamMembers).where(eq(teamMembers.employeeId, empId))
